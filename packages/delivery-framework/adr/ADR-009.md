...
The delivery framework manages 7+ types of governance artifacts (capabilities, user types, user stories, ROAD items, ADRs, NFRs, change entries) plus 4 DDD artifact types (bounded contexts, aggregates, value objects, domain events). Each artifact needs:
...
- `snake_case` → `camelCase` transformation happens in parsers, not schemas
