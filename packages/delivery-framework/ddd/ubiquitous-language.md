...
| **Keyword** | A term extracted from method titles and content, used for auto-linking scan findings to relevant methods. 625+ keywords are indexed. Normalized to lowercase for matching. | Field Guide | `byKeyword` in `MethodsIndexSchema` |
...
| **Methods Index** | The build-time JSON artifact containing all methods keyed by ID, byKeyword, byFieldGuide, byFramework, and byObservation reverse indices. Generated by `@foe/field-guide-tools` and baked into the scanner Docker image. | Field Guide | `MethodsIndexSchema` in `@foe/schemas/src/field-guide/methods-index.ts` |
...
| Term | Definition | Context(s) | Code Reference |
|------|-----------|------------|----------------|
| **Domain Model** | The top-level container for all DDD artifacts belonging to a single project. Aggregates bounded contexts, aggregates, value objects, domain events, and glossary terms. Identified by UUID. | Reporting | `TaxonomySnapshotSchema` in `@foe/schemas/src/taxonomy/taxonomy-snapshot.ts`, `domainModels` table |
| **Bounded Context** | A self-contained domain area with its own model, language, and responsibilities. Has slug, title, responsibility, source directory, team ownership, status (`draft \| stable \| deprecated`), subdomain type (`core \| supporting \| generic`, nullable), and relationships to other contexts. | Reporting | `BoundedContextSchema` in `@foe/schemas/src/taxonomy/extensions/bounded-context.ts`, `boundedContexts` table |
| **Subdomain Type** | A strategic DDD classification for bounded contexts: **Core** (key differentiator, invest heavily, build in-house), **Supporting** (necessary but not differentiating, integrate reliably), or **Generic** (commodity, buy/minimize). Nullable — `null` means unclassified. Drives investment decisions and build-vs-buy guidance. | Reporting | `SubdomainTypeSchema` in `@foe/schemas/src/taxonomy/extensions/bounded-context.ts` |
| **Aggregate** | A cluster of domain objects treated as a unit with a root entity. Has slug, root entity, child entities, value objects, events, commands, invariants, source file, and status (`draft \| implemented \| deprecated`). | Reporting | `AggregateSchema` in `@foe/schemas/src/taxonomy/extensions/aggregate.ts`, `aggregates` table |
| **Value Object** | An immutable domain primitive defined by its properties rather than identity. Has slug, typed properties, validation rules, immutability flag, and source file. | Reporting | `ValueObjectSchema` in `@foe/schemas/src/taxonomy/extensions/value-object.ts`, `valueObjects` table |
| **Domain Event** | A significant occurrence in the domain that other parts of the system may react to. Has slug, payload fields, consumers, triggers, side effects, and optional aggregate association. | Reporting | `DomainEventSchema` in `@foe/schemas/src/taxonomy/extensions/domain-event.ts`, `domainEvents` table |
| **Glossary Term** | A defined term within the ubiquitous language. Has term name, definition, aliases, examples, related terms, optional context scope, and source. | Reporting | `GlossaryTermSchema` in `@foe/schemas/src/taxonomy/extensions/glossary-term.ts`, `glossaryTerms` table |
| **Invariant** | A business rule that must always be true for an aggregate. Has description, enforcement flag, and optional enforcement location (code path). | Reporting | `InvariantSchema` in `@foe/schemas/src/taxonomy/extensions/aggregate.ts` |
| **Context Relationship** | A typed relationship between bounded contexts: `upstream \| downstream \| partnership \| shared-kernel \| separate-ways`. Includes communication pattern and description. | Reporting | `ContextRelationshipSchema` in `@foe/schemas/src/taxonomy/extensions/bounded-context.ts` |
| **Communication Pattern** | How two bounded contexts integrate: `domain-events`, `shared-kernel`, `anti-corruption-layer`, `open-host-service`, `conformist`, `partnership`, `customer-supplier`, or `separate-ways`. | Reporting | `CommunicationPatternSchema` in `@foe/schemas/src/taxonomy/extensions/bounded-context.ts` |
