...
export default function RoadmapCard({
  ...
  const getGovernanceColor = (type: "adr" | "bdd" | "nfr") => {
    switch (type) {
      ...
      case "bdd":
        return item.governance.bdd.status === "approved"
          ? "#10b981"
          : "#9ca3af";
      case "nfr":
  ...
  if (isMobile) {
    return (
      <div
        className={styles.roadmapCardMobile}
          ...
          {item.blocked_by && item.blocked_by.length > 0 && (
            <div className={styles.blockedSection}>
              ...
              {item.blocked_by.map((blocker) => (
                <span key={blocker} className={styles.blockedItem}>
                  {blocker}
                </span>
              ))}
            </div>
          ...
          {item.blocks && item.blocks.length > 0 && (
            <div className={styles.blocksSection}>
              ...
              {item.blocks.map((blocked) => (
                <span key={blocked} className={styles.blocksItem}>
                  {blocked}
                </span>
              ))}
            </div>
