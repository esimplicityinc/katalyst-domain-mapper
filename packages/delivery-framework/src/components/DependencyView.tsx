...
export default function DependencyView({
  items,
}: DependencyViewProps): JSX.Element {
      ...
      if (item.blocks && item.blocks.length > 0) {
        item.blocks.forEach((blockedId) => {
          ...
          const existingEdge = edges.find(
            (e) => e.source === item.id && e.target === blockedId,
          );
          if (!existingEdge) {
            edges.push({
  ...
  return (
    <div style={{ width: "100%", height: "600px" }}>
      ...
      >
        ...
        <MiniMap
          nodeColor={(node: Node): string =>
            (node.data?.color as string) || "#6b7280"
          }
      ...
      <div
        style={{ marginTop: "10px", fontSize: "0.875rem", color: "#6b7280" }}
      >
