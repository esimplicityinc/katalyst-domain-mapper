...
export async function buildMethodsIndex(): Promise<MethodsIndex> {
  ...
  const fieldGuideMethods = await glob(
    `${FIELD_GUIDES_ROOT}/*/methods/**/*.md`,
  );
  const frameworkMethods = await glob(
    `${EXTERNAL_FRAMEWORKS_ROOT}/*/methods/*.md`,
  );

  const allMethodFiles = [
    ...fieldGuideMethods.map((p) => ({
      path: p,
      source: "field-guide" as const,
    })),
    ...frameworkMethods.map((p) => ({ path: p, source: "framework" as const })),
  ];

  console.log(
    `Found ${allMethodFiles.length} method files (${fieldGuideMethods.length} field guide + ${frameworkMethods.length} framework)`,
  );
  ...
  for (const method of Object.values(methods)) {
    ...
    if (method.fieldGuide) {
      if (!byFieldGuide[method.fieldGuide])
        byFieldGuide[method.fieldGuide] = [];
