 ...
 */
export async function parseRoadItemFile(
  filePath: string,
): Promise<taxonomy.RoadItemExt> {
  ...
  const mapped = {
    ...
    governance: {
      adrs: {
        ...
        complianceCheck: (data.governance?.adrs?.compliance_check || []).map(
          (c: any) => ({
            adr: c.adr,
  ...
  try {
    return taxonomy.RoadItemExtSchema.parse(mapped);
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    throw new Error(
      `Road item ${data.id} validation failed in ${filePath}: ${message}`,
    );
  }
}
