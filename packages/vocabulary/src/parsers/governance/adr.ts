 ...
 */
export async function parseAdrFile(filePath: string): Promise<taxonomy.AdrExt> {
  ...
  try {
    return taxonomy.AdrExtSchema.parse(mapped);
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    throw new Error(
      `ADR ${data.id} validation failed in ${filePath}: ${message}`,
    );
  }
}
