...
export const subscoreMappings: PPTMapping[] = [
  ...
  {
    subscore: "Deployment Frequency",
    dimension: "feedback",
    category: "process",
  },
  { subscore: "Test Coverage", dimension: "feedback", category: "process" },
  {
    subscore: "Feedback Loop Investment",
    dimension: "feedback",
    category: "tech",
  },
  ...
  {
    subscore: "Architecture Clarity",
    dimension: "understanding",
    category: "process",
  },
  {
    subscore: "Domain Modeling",
    dimension: "understanding",
    category: "people",
  },
  {
    subscore: "Documentation Quality",
    dimension: "understanding",
    category: "people",
  },
  {
    subscore: "Code Organization",
    dimension: "understanding",
    category: "tech",
  },
...
export function calculatePPTMatrix(): PPTMatrix {
  ...
  > = {} as Record<PPTCategory, Record<string, MatrixCell>>;
  ...
  for (const dim of dimensions) {
    ...
    for (const cat of categories) {
      ...
      const matchedSubscores = relevantMappings
        .map((mapping) => {
          const subscore = dimData.subscores.find(
            (s) => s.name === mapping.subscore,
          );
          return subscore
            ? { name: subscore.name, score: subscore.score, max: subscore.max }
            : null;
        })
        .filter(
          (s): s is { name: string; score: number; max: number } => s !== null,
        );
  ...
  return [
    {
      ...
      pptBreakdown:
        currentScore <= 50 ? currentPPT : { people: 15, process: 20, tech: 15 },
    },
    {
      name: "Developing",
      range: "51-75",
      ...
      maxScore: 75,
      description:
        "Good practices established, optimization opportunities exist",
      pptBreakdown:
    ...
    },
    {
      ...
      pptBreakdown:
        currentScore > 75 ? currentPPT : { people: 85, process: 90, tech: 88 },
    },
  ];
}
