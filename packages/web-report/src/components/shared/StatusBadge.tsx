...
interface StatusBadgeProps {
  status: Severity | MaturityLevel | Confidence | 'immediate' | 'short-term' | 'medium-term';
  size?: 'sm' | 'md' | 'lg';
}
...
export function SeverityDot({ severity }: { severity: Severity }) {
  ...
  return (
    <span className={clsx('inline-block w-2 h-2 rounded-full', colors[severity])} />
  );
}

export function MaturityBadge({ level, score }: { level: MaturityLevel; score: number }) {
  const config = {
