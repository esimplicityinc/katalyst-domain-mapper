...
export default function Template8GitHubPage() {
  ...
  const [expandedDimensions, setExpandedDimensions] = useState<Set<string>>(new Set(['feedback']));
  const [selectedFile, setSelectedFile] = useState<{ dimension: DimensionKey; subscore: Subscore } | null>(null);

  const { dimensions, overallScore, maturityLevel, repository, scanDate, criticalFailures, recommendations, executiveSummary } = report;
  ...
  const getMergeStatus = () => {
    if (overallScore < 30) return { status: 'blocked', message: 'Blocked - Critical issues must be resolved', color: 'bg-red-600' };
    if (overallScore < 50) return { status: 'warning', message: 'Review required - Significant gaps identified', color: 'bg-yellow-500' };
    if (overallScore < 70) return { status: 'pending', message: 'Pending approval - Minor issues to address', color: 'bg-blue-500' };
    return { status: 'ready', message: 'Ready to merge - Meets quality standards', color: 'bg-green-500' };
  };

  const mergeStatus = getMergeStatus();

  const criticalChecks = criticalFailures.filter(f => f.severity === 'critical');
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-start gap-4">
            <div className={`p-2 rounded-full ${mergeStatus.status === 'blocked' ? 'bg-red-100' : mergeStatus.status === 'ready' ? 'bg-green-100' : 'bg-yellow-100'}`}>
              <GitPullRequest className={`w-6 h-6 ${mergeStatus.status === 'blocked' ? 'text-red-600' : mergeStatus.status === 'ready' ? 'text-green-600' : 'text-yellow-600'}`} />
            </div>
            <div className="flex-1">
              <h1 className="text-2xl font-semibold text-gray-900 flex items-center gap-3">
                FOE Assessment Report
                <span className={`px-2.5 py-0.5 rounded-full text-sm font-medium ${
                  mergeStatus.status === 'blocked' ? 'bg-red-100 text-red-800' :
                  mergeStatus.status === 'ready' ? 'bg-green-100 text-green-800' :
              ...
              <p className="text-gray-600 mt-1 flex items-center gap-4">
                ...
                <code className="px-2 py-0.5 bg-blue-100 text-blue-800 rounded text-sm font-mono">main</code>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <nav className="flex gap-6">
            ...
            >
              ...
              Files Changed
              <span className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full text-xs">
                {Object.values(dimensions).reduce((acc, d) => acc + d.subscores.length, 0)}
              </span>
            </button>
          </nav>
        </div>
          ...
          <div className="w-72 flex-shrink-0">
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
                <h3 className="font-semibold text-gray-900 text-sm">Assessment Dimensions</h3>
              </div>
              <div className="divide-y divide-gray-100">
                {(Object.entries(dimensions) as [DimensionKey, typeof dimensions.feedback][]).map(([key, dim]) => (
                  <div key={key}>
                    <button
                    ...
                    >
                      ...
                      <span className="flex-1 font-medium text-gray-700 text-sm">{dim.name}</span>
                      <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${
                        dim.score >= 60 ? 'bg-green-100 text-green-700' :
                        dim.score >= 40 ? 'bg-yellow-100 text-yellow-700' :
                    ...
                    {expandedDimensions.has(key) && (
                      <div className="bg-gray-50 border-t border-gray-100">
                        {dim.subscores.map((sub, idx) => (
                          <button
                            key={idx}
                            onClick={() => setSelectedFile({ dimension: key, subscore: sub })}
                          ...
                          >
                            ...
                            <span className={`text-xs ${
                              sub.score / sub.max >= 0.6 ? 'text-green-600' :
                              sub.score / sub.max >= 0.4 ? 'text-yellow-600' :
                              'text-red-600'
                            }`}>
                              {sub.score}/{sub.max}
                            </span>
              ...
              <div className="p-3 space-y-2">
                ...
                {criticalChecks.map((check, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-gray-700 truncate">{check.title}</span>
                  </div>
                ))}
                ...
                {passedChecks.length > 0 && (
                  <div className="flex items-center gap-2 text-sm pt-2 border-t border-gray-100">
                    ...
                    <span className="text-gray-600">{passedChecks.length} checks passed</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 border-b border-gray-200">
                    ...
                    <div>
                      <span className="font-semibold text-gray-900">FOE Scanner</span>
                      <span className="text-gray-500 text-sm ml-2">reviewed {scanDate}</span>
                    </div>
                  ...
                  <div className="p-4">
                    <div className="prose prose-sm max-w-none text-gray-700">
                      <p className="font-medium text-gray-900 mb-2">Executive Summary</p>
                ...
                {criticalFailures.map((failure, idx) => (
                  <div key={idx} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    <div className="flex items-center gap-3 px-4 py-3 bg-red-50 border-b border-red-100">
                      ...
                      <div className="flex-1">
                        <span className="font-semibold text-gray-900">FOE Scanner</span>
                        <span className="text-gray-500 text-sm ml-2">left a comment</span>
                      </div>
                    ...
                    <div className="p-4 border-b border-gray-100">
                      ...
                      <div className="space-y-2 text-sm">
                        <p><span className="font-medium text-gray-700">Evidence:</span> <span className="text-gray-600">{failure.evidence}</span></p>
                        <p><span className="font-medium text-gray-700">Impact:</span> <span className="text-gray-600">{failure.impact}</span></p>
                      </div>
                    </div>
                    <div className="px-4 py-3 bg-blue-50">
                      <p className="text-sm">
                        <span className="font-medium text-blue-700">Suggestion:</span>{' '}
                        <span className="text-blue-600">{failure.recommendation}</span>
                      </p>
                    </div>
                  </div>
                ))}
                  ...
                  <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 border-b border-gray-200">
                      ...
                      <span className="font-mono text-sm text-gray-700">
                        dimensions/{selectedFile.dimension}/{selectedFile.subscore.name.toLowerCase().replace(/\s+/g, '-')}.ts
                      </span>
                      <span className={`ml-auto text-sm font-medium ${
                        selectedFile.subscore.score / selectedFile.subscore.max >= 0.6 ? 'text-green-600' :
                        selectedFile.subscore.score / selectedFile.subscore.max >= 0.4 ? 'text-yellow-600' :
                        'text-red-600'
                      }`}>
                        {selectedFile.subscore.score}/{selectedFile.subscore.max} points
                      </span>
                    </div>
                    ...
                    <div className="font-mono text-sm">
                      ...
                      {selectedFile.subscore.evidence.length > 0 && (
                        <div className="border-b border-gray-100">
                          <div className="px-4 py-2 text-xs text-gray-500 bg-gray-50">Evidence Found:</div>
                      ...
                      {selectedFile.subscore.gaps.length > 0 && (
                        <div className="border-b border-gray-100">
                          <div className="px-4 py-2 text-xs text-gray-500 bg-gray-50">Gaps Identified:</div>
                      ...
                      {selectedFile.subscore.deductions.length > 0 && (
                        <div>
                          <div className="px-4 py-2 text-xs text-gray-500 bg-gray-50">Score Deductions:</div>
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-4 text-sm">
                                  <span className="text-green-600">+{evidenceCount}</span>
                                  <span className="text-red-600">-{gapCount}</span>
                                  ...
                                  <span className="w-12 text-right text-gray-600">{sub.score}/{sub.max}</span>
                                </div>
                              </button>
                            );
                          })}
      ...
      <footer className="border-t border-gray-200 bg-white mt-8">
        <div className="max-w-7xl mx-auto px-4 py-6 text-center text-gray-500 text-sm">
          FOE Assessment Report | Template 8: GitHub PR Review Style | Generated {scanDate}
        </div>
      </footer>
    </div>
  );
