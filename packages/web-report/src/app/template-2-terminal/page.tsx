...
function SectionHeader({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-amber-400 font-bold mt-8 mb-2">
      $ {children}
    </div>
  )
}
...
`}
      ...
      <div className="border-b border-gray-700 pb-4 mb-6">
        <div className="text-gray-400">Repository: <span className="text-white">{report.repository}</span></div>
        <div className="text-gray-400">Scan Date: <span className="text-white">{report.scanDate}</span></div>
        <div className="text-gray-400">Assessment Mode: <span className="text-amber-400">{report.assessmentMode}</span></div>
      </div>
      ...
      <div className="ml-4">
        <div className="text-2xl mb-2">
          Overall Score: <ProgressBar value={report.overallScore} max={100} width={30} />
        </div>
        <div className="text-gray-400">
          Maturity Level: <span className="text-amber-400">{report.maturityLevel}</span>
        </div>
      </div>
      ...
      <CodeBlock>
        <div className="text-gray-300 whitespace-pre-wrap">{report.executiveSummary}</div>
      </CodeBlock>
        ...
        {dimensions.map((dim) => (
          <div key={dim.name} className="border-l-2 border-gray-700 pl-4">
            ...
            <div className="mb-2">
              ...
              <span className="text-gray-500 ml-2">(confidence: {dim.confidence})</span>
            </div>
            
            {dim.subscores.map((sub) => (
              <div key={sub.name} className="ml-4 mb-2">
                <Prompt>
                  {sub.name}: <ProgressBar value={sub.score} max={sub.max} width={15} />
                </Prompt>
              </div>
            ))}
          </div>
      ...
      <div className="ml-4 space-y-4">
        {report.criticalFailures.map((failure) => (
          <div key={failure.id} className="border border-red-900/50 bg-red-950/20 rounded p-3">
            <div className="mb-1">
              {failure.severity === 'critical' ? <CriticalBadge /> : <HighBadge />}
            ...
            <CodeBlock>
              ...
              {failure.location && (
                <div className="text-gray-500 mt-1">Location: {failure.location}</div>
              )}
            </CodeBlock>
            ...
            <div className="text-gray-400 text-xs">
              Fix: <span className="text-green-400">{failure.recommendation}</span>
            </div>
          </div>
        ))}
      </div>
        ...
        {report.strengths.map((strength) => (
          <div key={strength.id}>
            ...
            <div className="ml-6 text-gray-400 text-xs">
              {strength.evidence}
              {strength.caveat && (
                <span className="text-amber-500"> (caveat: {strength.caveat})</span>
              )}
            </div>
          </div>
        ))}
        ...
        {report.gaps.map((gap) => (
          <div key={gap.id} className="border-l-2 border-amber-600 pl-3">
            <div className="text-amber-400">
              [-] {gap.title} <span className="text-gray-500">({gap.severity})</span>
            </div>
            <div className="text-gray-500 text-xs ml-4">{gap.evidence}</div>
            <div className="text-gray-400 text-xs ml-4">
              Recommendation: <span className="text-green-400">{gap.recommendation}</span>
            </div>
          </div>
        ))}
      </div>
          ...
          return (
            <div key={priority} className="mb-4">
              <div className={`font-bold mb-2 ${
                priority === 'immediate' ? 'text-red-400' :
                priority === 'short-term' ? 'text-amber-400' :
                'text-blue-400'
              }`}>
              ...
              {recs.map((rec, idx) => (
                <div key={rec.id} className="ml-4 mb-1">
                  ...
                  <span className="text-gray-500 ml-2">- {rec.description}</span>
                </div>
              ))}
            </div>
          )
      ...
      <CodeBlock>
        <div className="text-gray-300">
          <div>{'{'}</div>
          <div className="ml-4">"filesAnalyzed": {report.methodology.filesAnalyzed},</div>
          <div className="ml-4">"testFilesAnalyzed": {report.methodology.testFilesAnalyzed},</div>
          <div className="ml-4">"adrsAnalyzed": {report.methodology.adrsAnalyzed},</div>
          <div className="ml-4">"confidenceNotes": [</div>
          {report.methodology.confidenceNotes.map((note, idx) => (
            <div key={idx} className="ml-8 text-green-400">"{note}"{idx < report.methodology.confidenceNotes.length - 1 ? ',' : ''}</div>
          ))}
      ...
      <div className="mt-12 pt-4 border-t border-gray-700">
        <div className="text-green-400">
          scan complete. <span className="text-gray-500">{new Date().toISOString()}</span>
        </div>
        <div className="mt-2">
          $ <BlinkingCursor />
        </div>
