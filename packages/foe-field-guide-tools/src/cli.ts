...
program
  ...
  .action(async () => {
    try {
      ...
      const methodsJson = JSON.stringify(methodsIndex, null, 2);
      await writeFile(METHODS_INDEX_PATH, methodsJson);
      console.log(chalk.green(`✓ Methods: ${methodsIndex.stats.totalMethods} indexed (${(methodsJson.length / 1024).toFixed(1)} KB)`));

      // Observations
      const obsIndex = await buildObservationsIndex();
      const obsJson = JSON.stringify(obsIndex, null, 2);
      await writeFile(OBSERVATIONS_INDEX_PATH, obsJson);
      console.log(chalk.green(`✓ Observations: ${obsIndex.stats.totalObservations} indexed (${(obsJson.length / 1024).toFixed(1)} KB)`));
      ...
      console.log(chalk.green(`✓ Governance: ${govIndex.stats.totalRoadItems} roads, ${govIndex.stats.totalCapabilities} capabilities (${(govJson.length / 1024).toFixed(1)} KB)`));
...
program
  ...
  .description('Validate governance artifact frontmatter without building index')
  .action(async () => {
    try {
      ...
      const artifactTypes: Array<{ name: string; pattern: string; parser: (path: string) => Promise<unknown> }> = [
        { name: 'Capabilities', pattern: 'capabilities/CAP-*.md', parser: parseCapabilityFile },
        { name: 'Personas', pattern: 'personas/PER-*.md', parser: parsePersonaFile },
        { name: 'User Stories', pattern: 'user-stories/US-*.md', parser: parseUserStoryFile },
        { name: 'Use Cases', pattern: 'use-cases/UC-*.md', parser: parseUseCaseFile },
        { name: 'Road Items', pattern: 'roads/ROAD-*.md', parser: parseRoadItemFile },
        { name: 'ADRs', pattern: 'adr/ADR-*.md', parser: parseAdrFile },
        { name: 'NFRs', pattern: 'nfr/NFR-*.md', parser: parseNfrFile },
        { name: 'Change Entries', pattern: 'changes/CHANGE-*.md', parser: parseChangeEntryFile },
        { name: 'Bounded Contexts', pattern: 'ddd/contexts/*.md', parser: parseBoundedContextFile },
        { name: 'Aggregates', pattern: 'ddd/aggregates/*.md', parser: parseAggregateFile },
        { name: 'Value Objects', pattern: 'ddd/value-objects/*.md', parser: parseValueObjectFile },
        { name: 'Domain Events', pattern: 'ddd/events/*.md', parser: parseDomainEventFile },
      ];
  ...
  .action(async () => {
    try {
      ...
      for (const file of files) {
        try {
          ...
          if (status === 'adr_validated' && !hasValidatedAdrs) {
            warnings.push(`${road.id}: status is 'adr_validated' but adrs.validated is false`);
          }
          if (status === 'bdd_complete' && !hasBddScenarios) {
            warnings.push(`${road.id}: status is 'bdd_complete' but no BDD scenarios defined`);
          }
          const acceptableBddStates: string[] = ['approved', 'active'];
          if (status === 'bdd_complete' && !acceptableBddStates.includes(bddStatus)) {
            warnings.push(`${road.id}: status is 'bdd_complete' but BDD status is '${bddStatus}'`);
          }
          if (status === 'complete' && road.governance.nfrs.applicable.length > 0 && road.governance.nfrs.status === 'pending') {
            warnings.push(`${road.id}: status is 'complete' but NFR validation is still pending`);
          }

          // Check that valid next states exist (complete has none)
          const nextStates = governance.getNextStates(status);
          console.log(`  ${chalk.bold(road.id)}: ${status} → [${nextStates.join(', ') || 'terminal'}]`);
          validCount++;
        } catch (err: unknown) {
          const message = err instanceof Error ? (err as Error).message : String(err);
      ...
      if (warnings.length === 0) {
        ...
        console.log(chalk.yellow(`⚠ ${warnings.length} transition warnings:\n`));
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(chalk.green(`  Full coverage:    ${fullyCovered} (${total > 0 ? Math.round((fullyCovered / total) * 100) : 0}%)`));
      console.log(chalk.yellow(`  Partial coverage: ${partiallyCovered} (${total > 0 ? Math.round((partiallyCovered / total) * 100) : 0}%)`));
      console.log(chalk.red(`  No coverage:      ${uncovered} (${total > 0 ? Math.round((uncovered / total) * 100) : 0}%)`));
    } catch (err) {
  ...
  .action(async () => {
    try {
      ...
      for (const [perId, persona] of Object.entries(index.personas)) {
        ...
        const statusColor = persona.status === 'approved'
          ? chalk.green
      ...
      const approved = Object.values(index.personas).filter((p) => p.status === 'approved').length;
      const totalStories = Object.keys(index.userStories).length;
      const storiesWithPersona = new Set(Object.values(index.userStories).map((s) => s.persona));

      console.log('\n' + '-'.repeat(86));
      console.log(chalk.bold(`\nSummary: ${total} personas`));
      console.log(`  Approved: ${approved}/${total}`);
      console.log(`  Active personas (referenced by stories): ${storiesWithPersona.size}`);
