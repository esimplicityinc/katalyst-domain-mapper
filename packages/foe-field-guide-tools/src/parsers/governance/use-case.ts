 ...
 */
export async function parseUseCaseFile(filePath: string): Promise<governance.UseCase> {
  ...
  try {
    ...
    throw new Error(`Use case ${data.id} validation failed in ${filePath}: ${message}`);
  }
}
