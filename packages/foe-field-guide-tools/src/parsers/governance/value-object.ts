 ...
 */
export async function parseValueObjectFile(filePath: string): Promise<governance.ValueObject> {
  ...
  try {
    ...
    throw new Error(`Value object ${data.slug} validation failed in ${filePath}: ${message}`);
  }
}
