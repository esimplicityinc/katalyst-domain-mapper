 ...
 */
export async function parseAggregateFile(filePath: string): Promise<governance.Aggregate> {
  ...
  const mapped = {
    ...
    invariants: (data.invariants || []).map((inv: any) => ({
      description: inv.description || (typeof inv === 'string' ? inv : String(inv)),
  ...
  try {
    ...
    throw new Error(`Aggregate ${data.slug} validation failed in ${filePath}: ${message}`);
  }
}
