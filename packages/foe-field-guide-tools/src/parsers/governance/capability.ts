 ...
 */
export async function parseCapabilityFile(filePath: string): Promise<governance.Capability> {
  ...
  try {
    ...
    throw new Error(`Capability ${data.id} validation failed in ${filePath}: ${message}`);
  }
}
