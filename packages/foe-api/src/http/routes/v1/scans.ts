...
export function createScanRoutes(deps: {
  ...
  return new Elysia({ prefix: "/scans" })
    ...
    .post(
      ...
      {
        body: t.Object({
          repositoryPath: t.String({ description: "Absolute path to the repository to scan" }),
        }),
    ...
    .get(
      ...
      },
      {
        query: t.Object({
          status: t.Optional(
            t.String({ description: "Filter by status: queued, running, completed, failed" })
          ),
        }),
