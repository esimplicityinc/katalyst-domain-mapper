import { GovernanceRepository, GovernanceIndex } from '../../domain/governance/ports/GovernanceRepository';
import { db } from '../../db';
import { governance_snapshots, governance_capabilities, governance_road_items, governance_contexts } from '../../db/schema';
...
export class SqliteGovernanceRepository implements GovernanceRepository {
    ...
    async getSnapshots(): Promise<GovernanceIndex[]> {
        const results = await db.select().from(governance_snapshots).orderBy(desc(governance_snapshots.created_at));
    ...
    async deleteSnapshot(id: string): Promise<void> {
        ...
        await db.delete(governance_snapshots).where(eq(governance_snapshots.id, id));
    }
}
