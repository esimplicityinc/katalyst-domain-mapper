...
function buildContextPreamble(props: TaxonomyChatProps): string {
  return `[Context: You are helping manage the taxonomy architecture.
SNAPSHOT_ID: ${props.snapshotId ?? "none"}
Current state: ${props.nodeCount} taxonomy nodes, ${props.capabilityCount} capabilities.
...
export function TaxonomyChat(props: TaxonomyChatProps) {
  const { snapshotId, onUpdated: _onUpdated } = props;
  ...
  // Re-init chat when snapshotId changes
  useEffect(() => {
    initChat();
    return () => {
  ...
  const initChat = async () => {
    ...
    try {
      const session = await createDDDSession("Taxonomy Architecture Session");
      if (session) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await sendPromptAsync(sessionId, promptText, "taxonomy-architect", {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && !sending && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
              Taxonomy Architecture Chat
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 max-w-md mx-auto">
              Discover and refine your capability hierarchy with AI assistance
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {[
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-full hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors"
                >
                  {suggestion}
                </button>
              ))}
        ...
        {messages.map((msg) => (
          ...
          >
            {msg.role === "assistant" && (
              <div className="w-7 h-7 flex-shrink-0 flex items-center justify-center bg-green-100 dark:bg-green-900/30 rounded-full">
            ...
            >
              {msg.role === "assistant" ? (
                <div className="prose prose-sm dark:prose-invert max-w-none prose-p:my-1 prose-ul:my-1 prose-ol:my-1 prose-li:my-0.5 prose-headings:my-2 prose-pre:my-2 prose-code:text-green-600 dark:prose-code:text-green-400 prose-code:before:content-none prose-code:after:content-none">
        ...
        {pendingQuestion && !pendingQuestion.answered && (
          <div className="flex gap-3">
            ...
            <div className="max-w-[85%] rounded-lg border-2 border-green-300 dark:border-green-700 bg-white dark:bg-gray-800 overflow-hidden">
                          ...
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded-md border text-sm transition-colors ${
                              isSelected
                                ...
                                : "border-gray-200 dark:border-gray-600 hover:border-green-300 dark:hover:border-green-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
                            }`}
                          >
                    ...
                    {q.custom !== false && (
                      <div className="mt-2">
                        <input
                          ...
                          disabled={submittingQuestion}
                          className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-1 focus:ring-green-500 focus:border-green-500"
                        />
                      </div>
                    )}
                  </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center w-10 h-10 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white rounded-lg transition-colors flex-shrink-0"
          >
            {sending ? (
