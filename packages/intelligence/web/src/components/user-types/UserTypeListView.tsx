...
function UserTypeAvatar({
  name,
  archetype,
  size = "md",
}: {
  name: string;
  archetype: ManagedUserType["archetype"];
  size?: "sm" | "md" | "lg";
}) {
...
function StatusBadge({ status }: { status: ManagedUserType["status"] }) {
  const colors =
    status === "approved"
...
function UserTypeDetailPanel({
  ...
  userType: Partial<ManagedUserType> & { name: string };
  isNew: boolean;
  onSave: (data: Omit<ManagedUserType, "storyCount" | "capabilityCount">) => Promise<void>;
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[420px] bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 flex flex-col shadow-xl z-30">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
        <UserTypeAvatar name={form.name || "?"} archetype={form.archetype} size="sm" />
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white truncate">
            {isNew ? "New User Type" : form.name}
          </h3>
          {!isNew && (
            <p className="text-xs text-gray-500 dark:text-gray-400 font-mono">{userType.id}</p>
          )}
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select
              value={form.type}
              onChange={(e) => update("type", e.target.value as ManagedUserType["type"])}
              className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-1 focus:ring-rose-500"
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => update("archetype", e.target.value as ManagedUserType["archetype"])}
              className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-1 focus:ring-rose-500"
            >
        ...
        <div>
          ...
          <select
            value={form.status}
            onChange={(e) => update("status", e.target.value as ManagedUserType["status"])}
            className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-1 focus:ring-rose-500"
          >
                ...
                <select
                  ...
                  onChange={(e) =>
                    update("technicalProfile", {
                      ...
                      skillLevel: (e.target.value as ManagedUserType["technicalProfile"] & object extends { skillLevel?: infer S } ? S : never) || undefined,
                    })
                  }
                  className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-1 focus:ring-rose-500"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    update("technicalProfile", {
                      ...(form.technicalProfile ?? {}),
                      integrationType: (e.target.value as NonNullable<ManagedUserType["technicalProfile"]>["integrationType"]) || undefined,
                    })
                  }
                  className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-1 focus:ring-rose-500"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    update("technicalProfile", {
                      ...(form.technicalProfile ?? {}),
                      frequency: (e.target.value as NonNullable<ManagedUserType["technicalProfile"]>["frequency"]) || undefined,
                    })
                  }
                  className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-1 focus:ring-rose-500"
                >
        ...
        {/* Related User Types (read-only) */}
        {form.relatedUserTypes.length > 0 && (
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">
              Related User Types
            </label>
            <div className="flex flex-wrap gap-1">
              {form.relatedUserTypes.map((p) => (
                <span
                  key={p}
        ...
        {!isNew && (
          confirmDelete ? (
            <div className="flex items-center gap-2 p-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
              ...
              <button
                onClick={() => userType.id && onDelete(userType.id)}
                disabled={saving}
...
function UserTypeCard({
  ...
  const archetypeColor = ARCHETYPE_COLORS[userType.archetype] ?? "";
  const borderColor =
    userType.archetype === "creator"
      ? "border-emerald-200 dark:border-emerald-800"
      : userType.archetype === "operator"
        ? "border-blue-200 dark:border-blue-800"
        : userType.archetype === "administrator"
          ? "border-purple-200 dark:border-purple-800"
          : userType.archetype === "consumer"
    ...
    >
      <div className="p-4">
        <div className="flex items-start gap-3 mb-3">
          <UserTypeAvatar name={userType.name} archetype={userType.archetype} />
          <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-gray-700 pt-2">
          <span>{userType.storyCount} stories</span>
          <span>·</span>
          <span>{userType.capabilityCount} capabilities</span>
        </div>
      </div>
    </div>
  );
...
// ── Main UserTypeListView ─────────────────────────────────────────────────────

export function UserTypeListView() {
  const [userTypes, setUserTypes] = useState<ManagedUserType[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState<string | null>(null);
  const [archetypeFilter, setArchetypeFilter] = useState<string | null>(null);
  const [selectedUserType, setSelectedUserType] = useState<ManagedUserType | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const list = await api.listUserTypes();
      setUserTypes(list);
    } catch (err) {
  ...
  const filtered = userTypes.filter((p) => {
  ...
  const handleSave = async (data: Omit<ManagedUserType, "storyCount" | "capabilityCount">) => {
    setSaving(true);
    try {
      if (showNewPanel) {
        await api.createUserType(data);
        setShowNewPanel(false);
      } else if (selectedUserType) {
        await api.updateUserType(selectedUserType.id, data);
  ...
  const typeOptions: ManagedUserType["type"][] = ["human", "bot", "system", "external_api"];
  const archetypeOptions: ManagedUserType["archetype"][] = [
    "creator",
    "operator",
    "administrator",
    "consumer",
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Loading user types...</p>
        </div>
      </div>
    );
  }
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {userTypes.length === 0
                ? "No user types yet. Add the first one."
                : "No user types match your filters."}
            </p>
          </div>
        ) : (
