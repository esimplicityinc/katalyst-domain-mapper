...
function UserTypeBadge({
  ...
  userTypeId: string;
  userTypes: ManagedUserType[];
}) {
  const userType = userTypes.find((p) => p.id === userTypeId);
  if (!userType) {
    return (
  ...
  const bg = ARCHETYPE_AVATAR_COLORS[userType.archetype] ?? "bg-gray-400";
  const badge = ARCHETYPE_COLORS[userType.archetype] ?? "";
...
function StoryCard({
  ...
  return (
    ...
    >
      ...
      {/* User Type */}
      <div className="mb-2">
        <UserTypeBadge userTypeId={story.userType} userTypes={userTypes} />
      </div>
    ...
    <div className="fixed inset-y-0 right-0 w-[420px] bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 flex flex-col shadow-xl z-30">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">User Type</label>
            <select
              value={form.userType}
...
export function UserStoryBoardView() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [storyList, userTypeList] = await Promise.all([
        api.listUserStories(),
        api.listUserTypes(),
      ]);
  ...
  const filtered = stories.filter((s) => {
    if (userTypeFilter && s.userType !== userTypeFilter) return false;
  ...
  const activeUserType = userTypes.find((p) => p.id === userTypeFilter);
      ...
      <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          ...
          >
            ...
            <span>{activeUserType ? activeUserType.name : "All"}</span>
          ...
          {showUserTypeDropdown && (
            <>
              ...
              <div className="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-20 overflow-hidden">
                <button
                  onClick={() => { setUserTypeFilter(""); setShowUserTypeDropdown(false); }}
                ...
                {userTypes.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => { setUserTypeFilter(p.id); setShowUserTypeDropdown(false); }}
