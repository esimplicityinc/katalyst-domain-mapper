...
function buildContextPreamble(props: PersonaChatProps): string {
  return `[Context: You are helping manage personas and user stories.
Current state: ${props.personaCount} personas, ${props.storyCount} user stories, ${props.capabilityCount} capabilities available.
...
export function PersonaChat(props: PersonaChatProps) {
  const { onUpdated: _onUpdated } = props;
  ...
  // Stable session — one per browser session
  ...
  const initChat = async () => {
    ...
    try {
      const session = await createDDDSession("Persona & Story Session");
      if (session) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (isFirstMessageRef.current) {
        promptText = buildContextPreamble(props) + text;
        isFirstMessageRef.current = false;
      }

      await sendPromptAsync(sessionId, promptText, "persona-storyteller", {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && !sending && (
          <div className="text-center py-12">
            <Bot className="w-12 h-12 text-rose-400 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
              Persona &amp; Story Chat
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 max-w-md mx-auto">
              Discover personas and write user stories with AI assistance
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {[
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs text-rose-700 dark:text-rose-300 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 rounded-full hover:bg-rose-100 dark:hover:bg-rose-900/30 transition-colors"
                >
                  {suggestion}
                </button>
              ))}
        ...
        {messages.map((msg) => (
          ...
          >
            {msg.role === "assistant" && (
              <div className="w-7 h-7 flex-shrink-0 flex items-center justify-center bg-rose-100 dark:bg-rose-900/30 rounded-full">
            ...
            >
              {msg.role === "assistant" ? (
                <div className="prose prose-sm dark:prose-invert max-w-none prose-p:my-1 prose-ul:my-1 prose-ol:my-1 prose-li:my-0.5 prose-headings:my-2 prose-pre:my-2 prose-code:text-rose-600 dark:prose-code:text-rose-400 prose-code:before:content-none prose-code:after:content-none">
              ...
              {msg.streaming && (
                <div className="mt-1 flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-rose-400">Streaming...</span>
                </div>
              )}
            </div>
            {msg.role === "user" && (
        ...
        {pendingQuestion && !pendingQuestion.answered && (
          <div className="flex gap-3">
            ...
            <div className="max-w-[85%] rounded-lg border-2 border-rose-300 dark:border-rose-700 bg-white dark:bg-gray-800 overflow-hidden">
                          ...
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded-md border text-sm transition-colors ${
                              isSelected
                                ...
                                : "border-gray-200 dark:border-gray-600 hover:border-rose-300 dark:hover:border-rose-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
                            }`}
                          >
                    ...
                    {q.custom !== false && (
                      <div className="mt-2">
                        <input
                          ...
                          disabled={submittingQuestion}
                          className="w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-1 focus:ring-rose-500 focus:border-rose-500"
                        />
                      </div>
                    )}
                  </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center w-10 h-10 bg-rose-600 hover:bg-rose-700 disabled:bg-rose-400 text-white rounded-lg transition-colors flex-shrink-0"
          >
            {sending ? (
