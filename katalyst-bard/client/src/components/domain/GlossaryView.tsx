...
import { Plus, Loader2, BookOpen, Search, Pencil, Trash2, Check, X, ChevronDown, ChevronUp } from "lucide-react";
import { api } from "../../api/client";
import type { DomainModelFull, GlossaryTerm } from "../../types/domain";
import { DDDTooltip } from "./DDDTooltip";
// TODO: wire contribution system
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {sorted.map((t) => (
                <GlossaryRow
                  key={t.id}
                  term={t}
                  contextName={getContextName(t.contextId ?? null)}
                  model={model}
...
function GlossaryRow({ term, contextName, model, onModelUpdated }: GlossaryRowProps) {
  ...
  return (
    <>
      <tr
        className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${hasExpandableContent ? "cursor-pointer" : ""}`}
      ...
      {isExpanded && (
        <tr className="bg-gray-50/50 dark:bg-gray-800/30">
          <td colSpan={5} className="px-4 pb-4 pt-0">
            <div className="ml-5 pl-4 border-l-2 border-purple-200 dark:border-purple-800 space-y-2">
              {term.examples.length > 0 && (
                <div>
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-1.5">
                    {term.relatedTerms.map((rt, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 text-xs rounded-full"
                      >
                        {rt}
                      </span>
                    ))}
